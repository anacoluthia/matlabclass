...
hold on 
legend('Obs 1', 'Obs 2', 'Obs 3', 'location', 'NorthWest');
legend boxoff;
xlabel('Days');
ylabel('Unspecified units');
text(275,12, '1-2 corr = -0.476');
text(275,11, '1-3 corr = -0.724');
text(275,10, '2-3 corr = -0.172');
hold off
...
for i = 1:3;
    for j = i+1:3; % 2nd loop makes sure we don't look at duplicate correlations
        ...
        plot(lag, cAB);
        hold on 
        title(strcat('Lag - ',{' '},num2str([i,j],' Obs %d')));
        text(10,0.6,strcat('Lag =',{' '},num2str(maxLd(k),'%d days')));
        k = k+1;
        hold off
    end
end
